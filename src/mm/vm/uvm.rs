use alloc::boxed::Box;
use crate::{mm::{def::PGSZ, page_frame::alloc_page, pagetable_frame::PageTableFrame, pm::def::{TRAMPOLINE, TRAPFRAME}}, proc::process::Tcb, riscv::{registers::satp::Satp, sv39::pteflags::{PTE_FLAG_R, PTE_FLAG_V, PTE_FLAG_W, PTE_FLAG_X}}};
// User Virtual Memory
/// # Unsafety
...
impl Uvm {
    pub fn map_trap(&mut self,trapframe: usize) -> &mut Self{
        ...
        self.pagetables.mappages(TRAMPOLINE.into(), (trampoline as usize).into(), PGSZ, PTE_FLAG_V | PTE_FLAG_R | PTE_FLAG_X);
        self.pagetables.mappages(TRAPFRAME.into(),trapframe.into(), PGSZ, PTE_FLAG_R | PTE_FLAG_W | PTE_FLAG_X | PTE_FLAG_V);
        self
    }
    /// # Safety
