...
pub extern "C" fn usertrapret() {
    ...
    let task: alloc::sync::Arc<Tcb> = TASKMANAGER.lock().pop().expect("No Task in Manger");
    let pid = task.pid();
    ...
    let next_fn: usize = TRAMPOLINE + (userret as usize - trampoline as usize);
    unsafe {
